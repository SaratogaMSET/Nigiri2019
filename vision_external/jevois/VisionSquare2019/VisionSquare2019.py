...
class Vision2019:
    ...
    def detect(self, imgbgr, outimg = None):
        ...
        for c in contours:
            ...
            contour_matches.append((match, center, bound_rect_h))
            contour_costs.append((cost, center, bound_rect_h))
        ...
        target_heights = np.array([target_h for (_, _, target_h) in contour_matches[:2]])
        height = np.mean(target_heights, axis=0, dtype=np.float64)
        wh_ratio = width/height
        return centers, center, wh_ratio
    # ###################################################################################################
    ## Caclulate angle displacement
    def calculateAngle(self, w, h, wh_ratio):
        if wh_ratio == None:
            return
    ...
    def process(self, inframe, outframe):
        ...
        centers, center, wh_ratio = self.detect(imgbgr, outimg)

        if center != None:
            # Send serial messages
            angle = self.calculateAngle(w, h, wh_ratio)
        ...
        if abs(np.median(self.angle_data)) < ANGLE_THRESHOLD:
            jevois.sendSerial("LOCKED")
