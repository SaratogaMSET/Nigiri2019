...
public class AdvancedPIDBase extends SendableBase implements PIDInterface, PIDOutput {
  ...
  // The tolerance object for IZone
  private Tolerance m_iZone;
  ...
  public AdvancedPIDBase(double Kp, double Ki, double Kd, double Kf, PIDSource source,
                 PIDOutput output) {
    super(false);
   ...
   */
  @SuppressWarnings("ParameterName")
  public AdvancedPIDBase(double Kp, double Ki, double Kd, PIDSource source, PIDOutput output) {
  ...
  /**
   * Clear the integral term, but DON't disable the controller.
   */
   ...
   * @param absvalue absolute error which is tolerable in the units of the input object
   */
  public void setAbsoluteIZone(double absvalue) {
    m_thisMutex.lock();
    try {
      m_iZone = new AbsoluteTolerance(absvalue);
    } finally {
      m_thisMutex.unlock();
    }
  }
   ...
   * @param percentage percent error which is tolerable
   */
  public void setPercentIZone(double percentage) {
    m_thisMutex.lock();
    try {
        m_iZone = new PercentageTolerance(percentage);
    } finally {
      m_thisMutex.unlock();
    }
  }
